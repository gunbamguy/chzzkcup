...
function selectPlayer(card, playerId, playerFile, position) {
...
function assignPlayer(emptySlot) {
    ...
    // 드래프트 모드: 현재 차례인 팀만 선택 가능
    if (state.draftMode && teamId !== state.currentDraftTeam) {
        alert(`현재는 팀 ${state.currentDraftTeam + 1}의 차례입니다!`);
        return;
    }
    ...
    if (state.draftMode) {
        state.draftHistory.push({
            ...
            pick: state.draftHistory.length + 1,
            team: teamId,
            player: getPlayerName(state.selectedPlayer.file),
            position: position
        });
        console.log(`드래프트: 팀 ${teamId + 1} - ${getPlayerName(state.selectedPlayer.file)} (${POSITION_NAMES[position]})`);
    }
...
function assignPlayerByDrop(emptySlot, playerData) {
    ...
    if (state.draftMode) {
        state.draftHistory.push({
            ...
            pick: state.draftHistory.length + 1,
            team: teamId,
            player: getPlayerName(playerData.file),
            position: position
        });
        console.log(`드래프트: 팀 ${teamId + 1} - ${getPlayerName(playerData.file)} (${POSITION_NAMES[position]})`);
    }
...
function removePlayer(slot, player) {
    ...
    console.log(`[팀 ${teamId}] 선수 제거: ${position} 포지션`);
...
function resetAllTeams() {
    ...
    if (confirm('모든 팀을 리셋하시겠습니까? (선수 배치가 초기화되고 드래프트가 처음부터 시작됩니다)')) {
        ...
        try {
            ...
            // 드래프트 시작 버튼과 랜덤 버튼 다시 활성화
            const startBtn = document.getElementById('startDraftBtn');
            if (startBtn) {
                startBtn.textContent = '드래프트 시작';
            ...
            // 포지션 슬롯 초기화
            document.querySelectorAll('.position-slot:not(.jgl):not(.captain)').forEach(slot => {
                ...
                if (position) {
                    slot.innerHTML = '';
                    const emptySlotHtml = `<div class="empty-slot" data-position="${position}"></div>`;
            ...
            document.querySelectorAll('.team').forEach(team => {
                team.classList.remove('my-team', 'other-team', 'draft-turn');
            });
            ...
            console.log('모든 팀 리셋 완료 - 드래프트 재시작');
...
function updateDraftUI() {
    ...
    const currentPositionNameEl = document.getElementById('currentPositionName');
    const currentTeamNameEl = document.getElementById('currentTeamName');
    const startBtn = document.getElementById('startDraftBtn');
    const endBtn = document.getElementById('endDraftBtn');
    const draftStatus = document.getElementById('draftStatus');

    if (state.draftMode && state.draftStarted) {
        // 드래프트 진행 중
        if (currentRoundEl) currentRoundEl.textContent = state.draftRound;
        if (currentPositionNameEl) currentPositionNameEl.textContent = POSITION_NAMES[state.currentPosition];
        if (currentTeamNameEl) currentTeamNameEl.textContent = `팀 ${state.currentDraftTeam + 1}`;
        if (startBtn) {
        ...
        if (draftStatus) draftStatus.style.display = 'flex';
    } else {
        // 드래프트 시작 전
        if (startBtn) {
            startBtn.textContent = '드래프트 시작';
...
function updateTeamHighlight() {
    document.querySelectorAll('.team').forEach(team => {
        team.classList.remove('draft-turn');
    });

    if (state.draftMode) {
        const currentTeam = document.querySelector(`[data-team="${state.currentDraftTeam}"]`);
        if (currentTeam) {
            currentTeam.classList.add('draft-turn');
        }
    }
}
...
function handleStartDraft() {
    ...
    const startBtn = document.getElementById('startDraftBtn');
    if (startBtn) {
        startBtn.textContent = '드래프트 진행 중';
    ...
    saveToStorage();
    console.log('드래프트 시작!', '팀 순서:', state.teamOrder);
}

// 팀 순서 랜덤화 핸들러
function handleRandomizeTeamOrder() {
    if (state.draftStarted) {
        alert('드래프트가 이미 시작되었습니다. 팀 순서를 변경할 수 없습니다.');
        return;
    }

    randomizeTeamOrder();
    updateTeamOrderDisplay();
    saveToStorage();
    console.log('팀 순서 랜덤화:', state.teamOrder);
}
...
// 순서 변경 모드 토글
function handleToggleOrderEditMode() {
    if (state.draftStarted) {
        alert('드래프트 진행 중에는 순서를 변경할 수 없습니다.');
        return;
    }

    state.isOrderEditMode = !state.isOrderEditMode;

    const container = document.querySelector('.teams-container');
    const btn = document.getElementById('changeOrderBtn');
    const teams = document.querySelectorAll('.team');

    if (state.isOrderEditMode) {
        container.classList.add('order-edit-mode');
        if (btn) {
        ...
        container.classList.remove('order-edit-mode');
        if (btn) {
            btn.textContent = '순서 변경';
...
// DOM에서 현재 팀 순서 저장
function saveTeamOrderFromDOM() {
    const teams = Array.from(document.querySelectorAll('.team'));
    state.teamOrder = teams.map(team => parseInt(team.dataset.team));
    saveToStorage();
    console.log('팀 순서 저장됨:', state.teamOrder);
}

// 팀 순서 표시 업데이트
function updateTeamOrderDisplay() {
    // 팀 컨테이너에서 팀들을 순서대로 재배치
    const teamsContainer = document.querySelector('.teams-container');
    const teams = Array.from(document.querySelectorAll('.team'));
    ...
    state.teamOrder.forEach(teamId => {
        const team = teams.find(t => parseInt(t.dataset.team) === teamId);
        if (team) {
            teamsContainer.appendChild(team);
        }
    });
